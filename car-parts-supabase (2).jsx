...
// ════════════════════════════════════════════════════════════
// ⚙️ CONFIG — stored in localStorage, set via Setup page
// ════════════════════════════════════════════════════════════
const getConfig = () => ({
...
function SetupPage({ onDone, lang, t }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      else setErr(`Connection failed: ${res.status} — check URL and key`);
  ...
  return (
    <div style={{ background: "#0d0f14", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Noto Sans TC,sans-serif" }}>
      <style>{CSS}</style>
      <div style={{ width: "100%", maxWidth: 460, padding: 20 }}>
        <div style={{ textAlign: "center", marginBottom: 32 }}>
          ...
          <div style={{ color: "#64748b", fontSize: 13, marginTop: 4 }}>{t.setup}</div>
        </div>
        <div className="card" style={{ padding: 28 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22, padding: "10px 14px", background: "#0d0f14", borderRadius: 8, border: "1px solid #1e3a5f" }}>
            <span style={{ fontSize: 20 }}>🔗</span>
            <div><div style={{ fontSize: 13, fontWeight: 600, color: "#60a5fa" }}>Supabase Connection</div><div style={{ fontSize: 11, color: "#64748b" }}>Enter your project credentials below</div></div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: "#64748b", display: "block", marginBottom: 5 }}>{t.supabaseUrl}</label>
              <input type="url" placeholder="https://xxxx.supabase.co" value={url} onChange={e => setUrl(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: "#64748b", display: "block", marginBottom: 5 }}>{t.supabaseKey}</label>
              <input type="password" placeholder="eyJhbGci..." value={key} onChange={e => setKey(e.target.value)} />
              <div style={{ fontSize: 11, color: "#475569", marginTop: 4 }}>Settings → Data API → anon public key</div>
            </div>
            {err && <div style={{ color: "#fca5a5", fontSize: 12, background: "#7f1d1d33", padding: "8px 12px", borderRadius: 6 }}>❌ {err}</div>}
            <button className="btn btn-primary" style={{ width: "100%", padding: 10, fontSize: 14 }} onClick={handleSave} disabled={testing}>
              {testing ? "Testing connection..." : t.saveConfig}
            </button>
          </div>
        </div>
      </div>
