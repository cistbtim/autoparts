        ...
        <div className="card" style={{ padding: 28, boxShadow: "var(--shadow-lg)" }}>
          ...
          {authTab === "staff" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <p style={{ fontSize: 12, color: "var(--text3)", marginBottom: 6, textAlign: "center" }}>{t.demoAccounts}</p>
              {[{ role: "admin", u: "admin", p: "admin123" }, { role: "shipper", u: "shipper", p: "ship123" }].map(a => (
                <button key={a.role} className="btn btn-ghost btn-sm" style={{ width: "100%", marginBottom: 6, justifyContent: "space-between" }} onClick={() => { setUser(a.u); setPass(a.p); }}>
...
function MainApp({ user, onLogout, t, lang, setLang }) {
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "var(--bg)", minHeight: "100vh", color: "var(--text)" }}>
      ...
      {M.orderConfirm && (() => { const { order, phone, email } = M.orderConfirm; const msg = buildOrderMsg(order, lang); return (
        <div className="overlay" onClick={() => closeM("orderConfirm")}>
          <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 420 }}>
            ...
            <div style={{ background: "var(--surface2)", borderRadius: 12, padding: 14, marginBottom: 20, fontSize: 13, color: "var(--text2)", whiteSpace: "pre-line", lineHeight: 1.7, maxHeight: 180, overflowY: "auto" }}>{msg}</div>
            <p style={{ fontSize: 13, color: "var(--text3)", marginBottom: 14, textAlign: "center" }}>{lang === "zh" ? "傳送訂單確認給客戶：" : "Send order confirmation to customer:"}</p>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {phone && (
                <a href={waLink(phone, msg)} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
                  <button className="btn btn-primary" style={{ width: "100%", background: "#25D366", padding: 13, fontSize: 15 }}>
                    📲 {lang === "zh" ? "WhatsApp 傳給客戶" : "Send via WhatsApp"}
                  </button>
                </a>
              )}
              {email && (
                <a href={mailLink(email, `Order Confirmation - ${order.id}`, msg)} style={{ textDecoration: "none" }}>
                  <button className="btn btn-ghost" style={{ width: "100%", padding: 13, fontSize: 15 }}>
                    ✉ {lang === "zh" ? "Email 傳給客戶" : "Send via Email"}
                  </button>
                </a>
              )}
