...
const SHOP_WHATSAPP = "27834801815"; // YOUR WhatsApp number (no + or spaces)
const SHOP_EMAIL    = "cistbspeed@gmail.com"; // YOUR shop email
...
function MainApp({ user, onLogout, t, lang, setLang }) {
  ...
  const openM = (k, data) => setM(p => ({ ...p, [k]: { open: true, data: data ?? "__new__" } }));
  const closeM = (k) => setM(p => ({ ...p, [k]: { open: false, data: null } }));
  const isOpen = (k) => M[k]?.open === true;
  const mData = (k) => { const d = M[k]?.data; return d === "__new__" ? null : d; };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans',sans-serif", background: "var(--bg)", minHeight: "100vh", color: "var(--text)" }}>
      ...
      {isOpen("custHistory") && <CustHistoryModal customer={mData("custHistory")} orders={orders.filter(o => o.customer_phone === mData("custHistory")?.phone)} onClose={() => closeM("custHistory")} OC={ORDER_COLORS} />}
      ...
      {isOpen("orderConfirm") && (() => {
        const _oc = M.orderConfirm?.data || M.orderConfirm || {}; const { order, phone: custPhone, email: custEmail } = typeof _oc === 'object' && _oc.order ? _oc : (_oc.data || {});
        const msg = buildOrderMsg(order, lang);
        // Customer sends notification TO the shop — written from customer's point of view
        const items = Array.isArray(order?.items) ? order.items.map(i => `  • ${i.name} x${i.qty}  NT$${(i.price * i.qty).toLocaleString()}`).join("\n") : "";
        const shopMsg = lang === "zh"
          ? `您好！我想確認我的訂單 🛠️\n\n訂單編號：${order?.id}\n下單日期：${order?.date}\n\n我訂購的商品：\n${items}\n\n總金額：NT$${(order?.total || 0).toLocaleString()}\n\n我的聯絡資料：\n姓名：${order?.customer_name}\n電話：${order?.customer_phone}\n\n請確認收到，謝謝！`
          : `Hello! I'd like to confirm my order 🛠️\n\nOrder ID: ${order?.id}\nDate: ${order?.date}\n\nItems I ordered:\n${items}\n\nTotal: NT$${(order?.total || 0).toLocaleString()}\n\nMy contact:\nName: ${order?.customer_name}\nPhone: ${order?.customer_phone}\n\nPlease confirm receipt, thank you!`;
        return (
      ...
      {isOpen("rfqSend") && (() => { const _rq = M.rfqSend?.data || M.rfqSend || {}; const { part_name, part_sku, supplier_name, supplier_email, supplier_phone, qty_requested, token, message } = _rq.data ? _rq.data : _rq;
        const replyUrl = `${window.location.origin}${window.location.pathname}?rfq=${token}`;
