...
function MainApp({user,onLogout,t,lang,setLang,langs=[]}) {
  ...
  const deletePart=async(id)=>{const p=parts.find(pt=>pt.id===id);setBusyMsg(`Deleting ${p?.sku||""}${p?.name?" · "+p.name:""}`);const t0=Date.now();try{if(p)await logInv(p,p.stock,0,"Delete Part","Deleted");await Promise.all([api.delete("part_suppliers","part_id",id),api.delete("part_fitments","part_id",id)]);await api.delete("parts","id",id);setPartSuppliers(prev=>prev.filter(ps=>String(ps.part_id)!==String(id)));setPartFitments(prev=>prev.filter(f=>String(f.part_id)!==String(id)));await refreshTables("parts","inventory_logs");const ms=Date.now()-t0;showToast(`Deleted in ${ms<1000?(ms+"ms"):((ms/1000).toFixed(1)+"s")}`,"err");}finally{setBusyMsg(null);}};
