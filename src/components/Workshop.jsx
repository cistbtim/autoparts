        ...
        return(
          <div onClick={()=>setPhotoLightbox(null)}
            ...
            {canNav&&<button onClick={e=>{e.stopPropagation();setPhotoLightbox(idx-1);}}
              style={{position:"absolute",left:16,background:"rgba(255,255,255,.15)",border:"none",borderRadius:"50%",width:48,height:48,fontSize:24,cursor:"pointer",color:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>}
            {/* Image + AI panel */}
            ...
            {/* Next arrow */}
            {canNav&&<button onClick={e=>{e.stopPropagation();setPhotoLightbox(idx+1);}}
              style={{position:"absolute",right:16,background:"rgba(255,255,255,.15)",border:"none",borderRadius:"50%",width:48,height:48,fontSize:24,cursor:"pointer",color:"#ef4444",display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>}
          </div>
        );
      })()}
        ...
        !wsLocked&&<div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:14}}>
          ...
          {quoteItems.length>0&&!quoteItems.some(i=>i.type==="labour")&&(
            <div style={{background:"rgba(251,191,36,.15)",border:"1px solid rgba(251,191,36,.5)",borderRadius:6,padding:"7px 12px",fontSize:12,display:"flex",alignItems:"center",gap:6}}>
              <span>⚠️</span>
              <span>No labour or service fee added yet — did you forget to add one?</span>
            </div>
          )}
