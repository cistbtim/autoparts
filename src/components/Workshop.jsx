      ...
      {jobTab==="car"&&(
        <div className="card" style={{padding:16,marginBottom:14}}>
          ...
          <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
            ...
            <button className="btn btn-ghost btn-sm" onClick={()=>setShowLabelModal(p=>!p)} style={showLabelModal?{background:"var(--surface3)",color:"var(--accent)"}:{}}>🏷️ {t.wsLabel}</button>
          ...
          {/* Inline label frame */}
          {showLabelModal&&(()=>{
            const shopName2 = settings?.shop_name||"Workshop";
            const qr2 = `https://api.qrserver.com/v1/create-qr-code/?size=88x88&data=${encodeURIComponent(String(job.id||""))}&format=png`;
            const veh2 = [job.vehicle_make,job.vehicle_model,job.vehicle_year].filter(Boolean).join(" ");
            const doPrint = () => {
              ...
              win.document.write(`<!DOCTYPE html><html><head><title>Job Card Label</title><style>
                *{margin:0;padding:0;box-sizing:border-box}
                ...
                .lbl{color:#666;flex-shrink:0}.val{font-weight:bold;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
                ...
                .print-btn{padding:10px 28px;background:#1d4ed8;color:#fff;border:none;border-radius:6px;font-size:14px;font-weight:700;cursor:pointer}
                @media print{body{background:#fff;min-height:auto;gap:0;padding:6mm}.print-btn{display:none}}
              </style></head><body>
                <button class="print-btn" onclick="window.print()">🖨️ Print / Save PDF</button>
                <div class="label">
                  <img class="qr" src="${qr2}" alt="QR"/>
                  <div style="flex:1;min-width:0">
                    <div class="jobid">JOB #${job.id||""}</div>
                    <div class="shop">🔧 ${shopName2}</div>
                    <div class="reg">${job.vehicle_reg||"—"}</div>
                    ${veh2?`<div class="row"><span class="lbl">Vehicle:</span><span class="val">${veh2}</span></div>`:""}
            ...
            return (
              <div style={{marginBottom:14,padding:16,background:"var(--surface2)",borderRadius:10,border:"1px solid var(--border)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                  <div style={{fontWeight:700,fontSize:13}}>🏷️ Job Card Label</div>
                  <button className="btn btn-ghost btn-sm" onClick={()=>setShowLabelModal(false)}>✕</button>
                </div>
                <div style={{display:"flex",justifyContent:"center",marginBottom:12}}>
                  <div id="__job_label_print_root__" style={{width:340,border:"2px solid #000",borderRadius:8,padding:"12px 14px",display:"flex",gap:12,alignItems:"flex-start",background:"#fff",fontFamily:"Arial,sans-serif"}}>
                    <img src={qr2} alt="QR" style={{width:88,height:88,flexShrink:0,display:"block"}}/>
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{fontSize:11,fontFamily:"monospace",color:"#555",marginBottom:2}}>JOB #{job.id||""}</div>
                      <div style={{fontSize:9,textTransform:"uppercase",letterSpacing:".08em",color:"#888",marginBottom:4}}>🔧 {shopName2}</div>
                      ...
                      {veh2&&<div style={{fontSize:11,color:"#333",marginBottom:2}}><span style={{color:"#666"}}>Vehicle: </span><b>{veh2}</b></div>}
                ...
                <div style={{display:"flex",justifyContent:"flex-end"}}>
                  <button className="btn btn-primary btn-sm" onClick={doPrint}>🖨️ Print / Save PDF</button>
                </div>
              </div>
            );
          })()}
